...
var DropdownMenu = (function(){
  ...
  function initialise(){
    ...
    var menus = document.querySelectorAll('ul.dropdownMenu');
  ...
  function handleTouchStart(e){
    ...
    while (node != null){
      ...
      if (/\bdropdownMenu\b/.test(node.className)) return;
  ...
  function Menu(node){
    ...
    node.className += ' dropdownMenuJavaScript';
  ...
  Menu.prototype.handleMouseOver = function(e, immediate){
    ...
    // if top menu, open on click only
    if (!immediate) return;
  ...
  Menu.prototype.handleClick = function(e){
    ...
    if (item.nodeName == 'LI'){
      ...
      if (submenu.length > 0 && !/\bdropdownMenuOpen\b/.test(item.className)){
  ...
  Menu.prototype.open = function(){
    ...
    // change icon
    var span = this.getChildrenByTagName(this.toOpen, 'SPAN');
    
    if (span.length == 1){
        var imgs = this.getChildrenByTagName(span[0], 'IMG');
        if (imgs.length == 1) imgs[0].src = 'http://domain.com/images/list2.png';
    }
    ...
    for (var i = 0; i < items.length; i ++){
      ...
      if (submenu.length > 0){
        ...
        if (items[i] != this.toOpen){
          ...
          // close the submenu
          items[i].className =
              items[i].className.replace(/\bdropdownMenuOpen\b/g, '');
          this.close(items[i]);

        }else if (!/\bdropdownMenuOpen\b/.test(items[i].className)){

          // open the submenu
          items[i].className += ' dropdownMenuOpen';
          ...
          if (left < 0) items[i].className += ' dropdownMenuLeftToRight';

          // move the submenu to the left of the item if appropriate
          if (right > document.body.clientWidth){
            items[i].className += ' dropdownMenuRightToLeft';
          }
  ...
  Menu.prototype.close = function(node){
    ...
    if (this.toOpen){
        ...
        if (span.length == 1){
            var imgs = this.getChildrenByTagName(span[0], 'IMG');
            if (imgs.length == 1) imgs[0].src = 'http://domain.com/images/list.png';
        }
    }
    ...
    for (var i = 0; i < items.length; i ++){
      items[i].className = items[i].className.replace(/\bdropdownMenuOpen\b/g, '');
    }
